...
const edit = async (req, res) => {
  // Se extrae el ide de los parámetros
  const { id } = req.params;
  ...
  try {
    ...
    if (project === null) {
      log.info(`No se encontró el proyecto con el id: ${id}`);
      return res
        .status(404)
...
const editPut = async (req, res) => {
  ...
  try {
    ...
    req.flash('successMessage', 'Proyecto editado con éxito');
